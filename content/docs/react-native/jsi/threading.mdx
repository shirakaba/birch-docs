...
Here is a C++/WinRT example for React Native Windows (UWP), which launches the [root folder](https://learn.microsoft.com/en-us/uwp/api/windows.storage.applicationdata.localfolder? view=winrt-26100#windows-storage-applicationdata-localfolder) of the local app data store. It shows how we can cross over to the UI thread with uiInvoker to run a native UI API, then cross back to the JS thread to resolve a `Promise`.

### `MyHostObject.h`

We'll make a `MyHostObject` with a `launchRootFolder(): Promise<void>` API on it. It relies on being passed a `jsInvoker` and `uiInvoker` at construction time (which we'll show how to do in `ReactNativeModule.h`).
